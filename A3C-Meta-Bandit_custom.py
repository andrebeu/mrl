...
class Worker():
  ...
  def train(self,rollout,sess,gamma,bootstrap_value):
    ...
    """ 'Advantage' is just using value as baseline: Q(S,A) - V(S) 
    """
    ...
    with sess.as_default(), sess.graph.as_default():                 
      while not coord.should_stop():
        ...
        while d == False:
          ...
          action_distr,val,rnn_state_new = sess.run(
          ...
          action = np.random.choice(action_distr[0],p=action_distr[0])
          action = np.argmax(action_distr == action)

          rnn_state = rnn_state_new
          r,d,t = self.env.pullArm(action)
          episode_buffer.append([action,r,t,d,val[0,0]])
          episode_values.append(val[0,0])
          episode_frames.append(set_image_bandit(episode_reward,self.env.bandit,action,t))
          episode_reward[action] += r
          total_steps += 1
